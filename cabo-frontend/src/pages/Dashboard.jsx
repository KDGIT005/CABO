...
import api from '../api';
import {
  Car, Users, Bell, MapPin, ArrowRight,
  Calendar, Clock, CheckCircle, LayoutDashboard
} from 'lucide-react';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {rides.map(ride => (
          <Link to={`/rides/${ride.id}`} key={ride.id} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div className="card" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, cursor: 'pointer' }}>
              <div>
                <div className="ride-route" style={{ marginBottom: 6 }}>
                  <MapPin size={13} />
                  {ride.fromLocation}
                  <ArrowRight size={13} className="ride-route-arrow" />
                  {ride.toLocation}
                </div>
                <div style={{ display: 'flex', gap: 16, fontSize: '0.8rem', color: 'var(--text-secondary)', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Calendar size={11} /> {formatDate(ride.date)}
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Clock size={11} /> {formatTime(ride.time)}
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Users size={11} /> {ride.participantCount || 0} riders
                  </span>
                  {ride.pricePerSeat > 0 && (
                    <span style={{ color: 'var(--accent-teal)', fontWeight: 700 }}>
                      ₹{Math.round(ride.pricePerSeat)}/person
                    </span>
                  )}
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{ride.carModel}</span>
              </div>
            </div>
          </Link>
        ))}
      ...
      <div className="container">
        ...
        <div className="page-header">
          ...
          <p className="page-subtitle">
            Hey, {user?.name?.split(' ')[0]} 👋 — manage your rides and notifications
          </p>
        </div>
              ...
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                {notifications.map(n => (
                  <div key={n.id} className={`notification-item ${!n.read ? 'unread' : ''}`}>
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{ margin: 0, fontSize: '0.875rem', fontWeight: n.read ? 400 : 600 }}>{n.message}</p>
